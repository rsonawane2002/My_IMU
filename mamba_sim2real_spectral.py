...
class VariableODRDataset(IterableDataset):
    ...
    def _process_pair(self, sim_csv, real_csv):
        ...
        # 2. ODR Augmentation Logic Fix
        step_size = 1 
        if self.training:
            step_size = np.random.randint(1, 5)
            # CRITICAL: Slice BOTH dataframes to keep them row-aligned
            sim_df = sim_df.iloc[::step_size].reset_index(drop=True)
            real_df = real_df.iloc[::step_size].reset_index(drop=True)

        # Since data is pre-fixed at 400Hz (0.0025s), use a stable constant
        current_dt = 0.0025 * step_size

        # 3. Sim Features
        joint_cols = ['j1', 'j2', 'j3', 'j4', 'j5', 'j6', 'j7']
        # Use current_dt for the velocity calculation for numerical stability
        sim_vels = sim_df[joint_cols].diff().fillna(0) / current_dt
        ...
        # 4. Length Alignment
        min_len = len(sim_features_orig) # They are now guaranteed to match
        if min_len < self.seq_len: 
            return None, None

        # 5. Target Generation
        real_acc = real_df[['acc_x_g', 'acc_y_g', 'acc_z_g']].values[:min_len]
        real_gyro = real_df[['gyro_x_rad_s', 'gyro_y_rad_s', 'gyro_z_rad_s']].values[:min_len]
        real_imu = np.hstack([real_acc, real_gyro])
       
        # Slicing is now just to match array lengths exactly
        sim_imu = sim_features_orig[:min_len, :6]
        dt_col = np.full((min_len, 1), current_dt)
        X = np.hstack([sim_features_orig[:min_len], dt_col])
        Y = real_imu - sim_imu
        Y = np.clip(Y, -20, 20)                           
...
class MambaSeq2Seq(nn.Module):
    ...
    def forward(self, x_full):
        ...
        log_var = torch.clamp(log_var, min=-1, max=5)
        return mu, log_var
...
class Sim2RealLoss(nn.Module):
    ...
    def forward(self, mu, log_var, target):
        ...
        sampled_pred = mu + (epsilon * std)
        fft_pred = torch.fft.rfft(sampled_pred, dim=1)
...
def match_files(data_dir):
    # Change pattern back to the original raw simulation files
    search_pattern = os.path.join(data_dir, "**", "imu_*.csv")
    all_files = sorted(glob.glob(search_pattern, recursive=True))
    pairs = []
    print(f"REVERTING: Scanning {data_dir} for ORIGINAL raw pairs...")
   
    for sim_path in tqdm(all_files):
        filename = os.path.basename(sim_path)
        # Ensure we DON'T pick up the 400hz files we just made
        if "400hz" in filename or filename.startswith("real_"): 
            continue
            
        file_id = filename.replace("imu_", "").replace(".csv", "")
        real_path = os.path.join(os.path.dirname(sim_path), f"real_imu_{file_id}.csv")
...
NEW
def match_files(data_dir):
    # Specifically target the 400hz simulation files
    search_pattern = os.path.join(data_dir, "**", "imu_400hz_*.csv")
    all_files = sorted(glob.glob(search_pattern, recursive=True))
    pairs = []
    print(f"Scanning {data_dir} for 400Hz normalized pairs...")
   
    for sim_path in tqdm(all_files):
        filename = os.path.basename(sim_path)
        # Construct the matching real file name
    ...
    print(f"Found {len(pairs)} perfect 400Hz pairs.")
    return pair
'''
...
if __name__ == "__main__":
    ...
    #CHANGED: lower to less fft_weight
    #criterion = Sim2RealLoss(fft_weight=0.1).to(device)
    criterion = Sim2RealLoss(fft_weight=0.0001).to(device)
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        for x, y in pbar:
            ...
            # Update pbar with Loss, LR, AND Sigma
            pbar.set_postfix({
                'loss': f"{loss.item():.4f}",
                'lr': f"{curr_lr:.6f}",
