...
class VariableODRDataset(IterableDataset):
    ...
    def _process_pair(self, sim_csv, real_csv):
        ...
        if sim_df.isnull().values.any() or real_df.isnull().values.any():
            return None, None
        ...
        sim_vels_np = signal.savgol_filter(
        ...
        sim_features_orig = np.hstack([
            sim_imu_raw,
            sim_df[joint_cols].values,
            sim_vels_np # Using smoothed velocity
        ])
...
class MambaSeq2Seq(nn.Module):
    ...
    def forward(self, x_full):
        ...
        print("dt_vals", dt_vals.shape)
        print("x_emb", x_emb.shape)
        print("mamba_out", mamba_out.shape)
        print("raw output", raw_output.shape)
        print("mu", mu.shape)
...
class Sim2RealLoss(nn.Module):
    ...
    def forward(self, mu, log_var, target):
        ...
        print(f"\n[Loss Debug] mu shape: {mu.shape}")
        print(f"[Loss Debug] log_var shape: {log_var.shape}")
        print(f"[Loss Debug] target shape: {target.shape}")
        print(f"[Loss Debug] FFT pred shape: {fft_pred.shape}")
        print(f"[Loss Debug] FFT target shape: {fft_target.shape}")
...
def match_files(data_dir):
    # Update search pattern to match your new naming convention
    ...
    for sim_path in tqdm(all_files):
        ...
        # Extract the unique ID (the part after imu_400hz_)
        # Example: imu_400hz_trial_01.csv -> trial_01
        file_id = filename.replace("imu_400hz_", "").replace(".csv", "")
        
        # Construct the expected real filename
...
def fit_scaler(file_pairs):
    ...
    for idx in indices:
        ...
        try:
            ...
            joint_cols = ['j1', 'j2', 'j3', 'j4', 'j5', 'j6', 'j7']
            dt_sim = 0.016
            vels_np = signal.savgol_filter(
                df[joint_cols].values, window_length=7, polyorder=3, deriv=1, delta=dt_sim, axis=0
            )
           
            imu_cols = ['imu_ax', 'imu_ay', 'imu_az', 'imu_gx', 'imu_gy', 'imu_gz']
            feats = np.hstack([df[imu_cols].values, df[joint_cols].values, vels_np])
...
if __name__ == "__main__":
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        for x, y in pbar:
            ...
            print("Debug prints complete. Exiting safely.")
            exit()
